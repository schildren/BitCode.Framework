...
| Servicio interno (futuro) | Comunicación entre bounded contexts extraídos (Fase 9) o publicador/consumidor de eventos (Fase 3, Kafka) | No existe todavía en runtime — ADR 0005 `Accepted` (2026-09-06), adapter Kafka en construcción desde F3-02; se modela como amenaza futura hasta que exista código de adapter. |
...
[Sample.Api / API consumidora] ── usa ──> [Shared.Infrastructure.Web: GlobalExceptionHandler, ToOkOrProblem]
        ...
        │  (3) API ↔ resolución de tenant (ITenantProvider — HttpContextTenantProvider desde claim JWT, F1-12)
        ▼
...
| S2 | Suplantar a otro tenant si la resolución de `TenantId` confía en un dato controlable por el cliente (p. ej. un header HTTP sin validar) | (3) | Antes de F1-12: el framework no proveía una implementación productiva de `ITenantProvider` que resolviera el tenant desde el JWT validado — solo existía `NullTenantProvider` (deshabilita multi-tenancy) y `FakeTenantProvider` en tests | **Mitigado (F1-12).** `HttpContextTenantProvider` (`src/Shared.Infrastructure.Web/MultiTenancy/HttpContextTenantProvider.cs`) resuelve el `TenantId` exclusivamente desde el claim `TenantClaimTypes.TenantId` (`src/Shared.Domain/MultiTenancy/TenantClaimTypes.cs`) de `HttpContext.User` — nunca desde `HttpContext.Request` (headers/query string) — y ese claim viaja firmado dentro del JWT emitido por `JwtTokenGenerator`. Un usuario autenticado sin ese claim provoca `TenantResolutionException` (nunca un tenant por defecto). Probado con `HttpContextTenantProviderTests` (unitario, incluye el caso explícito de que un header/query string manipulado se ignora) y con `MultiTenantDbContextIntegrationTests` (SQL Server real: aislamiento de listado y de acceso puntual por Id entre tenants). Riesgo residual: `docs/convenciones.md` documenta ahora cuándo usar `HttpContextTenantProvider` vs. `NullTenantProvider`, pero un proyecto consumidor que implemente su propio `ITenantProvider` desde cero (en vez de reutilizar el del framework) sigue siendo responsable de aplicar el mismo principio. |
...
| S4 | Suplantación de identidad entre servicios cuando existan servicios extraídos (Fase 9) sin mTLS/workload identity | (10, futuro) | ADR 0004 fija "Client Credentials/workload identity y mTLS cuando existan servicios separados" como dirección, no como hecho | **Planeada** — Fase 2 (F2-01 a F2-06) y Fase 9 (extracción de microservicios). No aplica hoy porque no hay servicios separados en runtime. Para la frontera (10) específica (servicio ↔ Kafka), F3-11 ya deja la *configuración* de identidad SASL/mTLS lista (`KafkaMessagingOptions`/`KafkaClientConfigFactory`, `docs/politica-seguridad-kafka.md`), pero sigue siendo `Planeada` en el sentido de que ningún broker real (staging/producción) la aplica todavía — verificación contra broker SASL/ACL real pendiente del runbook de ese despliegue. |
...
| I5 | Fuga de datos entre tenants por contaminación de cache: dos tenants que piden el mismo recurso lógico con la misma clave literal contra `HybridCache` reciben el mismo valor cacheado si la clave no incluye el `TenantId` | (6) | Hasta F1-16, `Shared.Infrastructure.Caching` registraba `HybridCache` sin ningún mecanismo de tenant-scoping y `docs/convenciones.md` solo documentaba `HybridCache.GetOrCreateAsync` directo, sin advertir sobre este vector. Demostrado empíricamente en `TenantAwareCacheTests.RawHybridCache_WithSameLiteralKey_LeaksValueAcrossTenants` (dos "tenants" comparten la misma instancia de `HybridCache` y la misma clave literal: el segundo recibe el valor cacheado por el primero) | **Mitigado (F1-16).** `ITenantAwareCache`/`TenantAwareCache` (`src/Shared.Infrastructure.Caching`, registrado por `AddSharedCaching` vía `TryAddScoped`) compone el `TenantId` resuelto por `ITenantContext` (F1-15) en la clave efectiva (`"tenant:{TenantId}:{key}"`) y lanza `TenantResolutionException` si el proyecto opera en modo multi-tenant sin un `TenantId` resuelto para el scope — nunca cachea con una clave ambigua. Probado en `TenantAwareCacheTests` (Shared.Infrastructure.Caching.Tests): mismo recurso lógico cacheado por dos tenants distintos contra la misma instancia de `HybridCache` no se cruza. Riesgo residual: un handler que siga usando `HybridCache` directamente (en vez de `ITenantAwareCache`) para datos sensibles a tenant sigue expuesto — depende de revisión de código, no hay todavía un architecture test que lo prohíba automáticamente (mismo patrón de hueco de enforcement que T2/I1). |
...
1. ~~**S2**~~ — **Mitigado en F1-12** (`HttpContextTenantProvider` + `TenantClaimTypes` + `TenantResolutionException` + guía en `docs/convenciones.md`). Riesgo residual acotado a proyectos consumidores que implementen su propio `ITenantProvider` sin reutilizar el del framework.
...
11. **I5 (residual tras F1-16)** — `ITenantAwareCache` cierra la contaminación de cache entre tenants para quien lo use, pero no hay architecture test que impida que un handler nuevo siga llamando `HybridCache` directamente con una clave sin tenant para datos sensibles — depende de disciplina de code review, mismo patrón de hueco que T2/I1.
