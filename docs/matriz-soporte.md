...
| Broker — Kafka | — | — | Planeado, sin código | ADR [0005](adr/0005-mensajeria-kafka.md), estado `Proposed` (no `Accepted`). No existen los proyectos `BitCode.Messaging.Contracts`/`BitCode.Messaging.Kafka` previstos por el Plan Maestro, ni fixtures de Testcontainers para Kafka, ni jobs de CI que lo ejerciten. Corresponde a Fase 3 |
...
- **Kafka:** no hay código ni test; el ADR 0005 está en estado `Proposed`, pendiente de aprobación humana explícita (Plan Maestro, sección 13, "introducción de una nueva base de datos o broker") antes de construirse en Fase 3.
