...
- [ ] Los paquetes tienen versionado, firma y provenance. **Parcial, no cumplido todavía.** El mecanismo (MinVer/Nx + SLSA `attest-build-provenance`) está implementado. El bloqueo de licencia se resolvió el 2026-09-12: `docs/adr/0008-licencias-open-core.md` pasó a `Accepted` con aprobación humana explícita (Javier León), y el repositorio ya tiene `LICENSE` (núcleo, Apache-2.0) y `LICENSE-ENTERPRISE.md` (capa propietaria interina) — ver `NOTICE.md` para el límite exacto. Sigue sin cumplirse por dos motivos independientes de la licencia: (1) la firma de paquetes NuGet depende de `NUGET_SIGNING_CERTIFICATE`, un secreto que no existe en el repositorio — el paso fallaría si se disparara hoy; (2) `release.yml`/`nuget-publish.yml`/`npm-publish.yml` nunca corrieron de punta a punta en modo real (solo existe el tag `v0.1.0`, sin release publicado). Pendiente: provisionar el secreto de firma y ejercitar al menos un dry-run exitoso del pipeline de release antes de marcar este criterio.
