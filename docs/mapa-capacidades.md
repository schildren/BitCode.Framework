...
| 18 | Mensajería basada en eventos (Kafka, Outbox) | Contratos agnósticos de proveedor implementados (F3-01: `IIntegrationEvent`/`IEventPublisher`/`IEventConsumer`); ADR 0005 `Accepted` (2026-09-06); adapter Kafka, proyectos `BitCode.Messaging.*` y fixtures de Testcontainers para Kafka aún no implementados (`docs/matriz-soporte.md`) | Toda la integración orientada a eventos entre bounded contexts (decisión rectora del Plan Maestro, sección 2) depende de esta capacidad | Alta | Fase 3 — Plataforma de eventos |
