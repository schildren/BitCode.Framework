...
- `Version` = `0.1.0` para todos — no existe ninguna versión previa publicada (`docs/politica-versionado.md`, sección 2, confirma que hoy no hay ningún paquete NuGet propio publicado); es la primera versión pre-1.0 según la política de SemVer ya vigente en ese documento. La política de versionado independiente por proyecto (sección 2 de `docs/politica-versionado.md`) aplica desde el próximo cambio real; hoy todos parten del mismo punto de partida porque ninguno tuvo una versión previa.
