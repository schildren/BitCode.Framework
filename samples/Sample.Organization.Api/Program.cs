...
// EnsureCreated en vez de migraciones (mismo criterio que Sample.Api/Sample.IdentityAdmin.Api): este es
// un proyecto de referencia/demo de la plataforma, no un consumidor productivo -- ningún DbContext del
// repositorio usa `dotnet ef migrations` todavía (ver docs/guia-organization.md, sección "Pendientes").
using (var scope = app.Services.CreateScope())
{
    await scope.ServiceProvider.GetRequiredService<OrganizationDbContext>().Database.EnsureCreatedAsync();
